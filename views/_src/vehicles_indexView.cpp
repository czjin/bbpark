...
class T_VIEW_EXPORT vehicles_indexView : public TActionView
{
  Q_OBJECT
public:
  vehicles_indexView() : TActionView() { }
  vehicles_indexView(const vehicles_indexView &) : TActionView() { }
  QString toString();
};

QString vehicles_indexView::toString()
{
  responsebody.reserve(2074);
  ...
  tfetch(QList<Vehicles>, vehiclesList);
  for (const auto &i : vehiclesList) {;
...
Q_DECLARE_METATYPE(vehicles_indexView)
T_REGISTER_VIEW(vehicles_indexView)

#include "vehicles_indexView.moc"
